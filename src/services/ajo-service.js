...
export function mapAjo(raw = {}) {
  ...
  return {
    ...
    joined: Boolean(raw.joined ?? raw.isMember ?? viewer.isMember ?? viewer.member),
    canManage: Boolean(raw.canManage ?? viewer.canManage ?? viewer.isCreator),
...
export const ajoService = {
  ...
  async listForViewer(filters = {}) {
    const page = await this.list(filters);
    const details = await Promise.allSettled(page.items.map((ajo) => this.detail(ajo.id, ajo)));
    return {
      ...page,
      items: page.items.map((ajo, index) => (
        details[index].status === "fulfilled"
          ? mergeAjoSummaryWithDetail(ajo, details[index].value)
          : ajo
      )),
    };
  },
