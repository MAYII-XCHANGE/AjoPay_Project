...
export function AdminSupportIssuesPage() {
  ...
  return (
    <div className="admin-support">
      ...
      {issues.isLoading ? <Skeleton className="skeleton--table" /> : issues.data?.length ? <div className="support-grid">{issues.data.map((issue) => <IssueCard issue={issue} admin onReview={openReview} key={issue.id} />)}</div> : <Card><EmptyState icon={<AlertIcon />} title="No open system issues" text="Everything is operating normally." /></Card>}
